...
file_name = "sherlock-holmes-bge-large-en-v1-5"
...
if calculate_embedding:
    ...
    for query in queries:
        similarity_scores.append(return_similarity_scores(
            chunks, "json", f"./week-3/chatbot-cmd-class/generated/{file_name}.json", query, 3))

    from core.data import Data
    from core.chat import Chat
    chat1 = Chat("query", Data.user_info,
                 None, Data.assistant_chat_params, 0)
    for i in range(len(similarity_scores)):
        print(queries[i])
        texts = []
        for similarity in similarity_scores[i]:
            texts.append(similarity["text"])
        answer = chat1.generate_output(queries[i], texts)
        print(f"{answer}\n")
